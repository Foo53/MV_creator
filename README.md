...
LLM を複数のエージェントとして段階的に呼び出し、Suno 用の歌詞・style、楽曲構成、映像方針、脚本、キャラクター、シーン、ショット、画像生成プロンプトを生成します。
...
- 楽曲構成に合わせた MV の映像方針、脚本、キャラクター、シーン、ショットを生成
- ショットごとの画像生成プロンプトと動画編集プロンプトを生成
...
mv-creator create-mv --project <name> --idea <text> [--provider mock|gemini|claude]
mv-creator create-mv-from-lyrics --project <name> --lyrics <text> [--music-style <text>] [--provider mock|gemini|claude]
mv-creator revise --project <name> [--provider mock|gemini|claude]
mv-creator rebuild-mv-visuals --project <name> [--provider mock|gemini|claude]
...
`create-mv` では `--audience`、`--style`、`--duration-seconds`、`--genre`、`--mood`、`--color-tone`、`--narration-style`、`--target-platform` も指定できます。

TikTok や Instagram Reel を指定すると、タイムラインは縦長の `1080x1920` になります。
...
| `video_prompts.md` | Remotion のパン・ズーム、ホールド、クロスフェード用編集メモ |
...
- タイムラインの尺は制作ブリーフの想定尺へ正規化されます。アップロードした BGM ファイル自体の再生時間は解析しないため、必要に応じて制作ブリーフまたは `timeline_manifest.json` を調整してください。
...
- Remotion の SE と読み上げ音声トラックは未実装です。
