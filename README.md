...
MV Creator は、短いアイデアから歌詞字幕付きの静止画MVを組み立てる Python 製ツールです。
LLM を段階的に呼び出し、Suno 用の歌詞・style、楽曲構成、歌詞や雰囲気に合う画像スライド、画像生成プロンプトを生成します。
...
- 歌詞セクションに合わせた画像スライドを一度に設計
- 画像ごとの ChatGPT 用プロンプトと静止画MV編集メモを生成
...
| `design.md` | 制作ブリーフ、音楽、画像スライド設計 |
| `storyboard.md` | 画像スライドごとの構成 |
| `image_prompts.md` | 画像生成プロンプト |
| `editing_prompts.md` | Remotion のパン・ズーム、ホールド、クロスフェード用編集メモ |
| `continuity_report.md` | 互換用の継続性レポート |
| `rag_store.json` | 互換用の簡易 RAG ストア |
| `rag_trace.md` | 互換用の RAG 参照履歴 |
